...
type ShardedFileStore struct {
	...
	ExifRemove           bool          // Should attempt to remove exif data
...
func (upload *fileUpload) FinishUpload(ctx context.Context) error {
	...
	// If upload is an image file attempt to remove exif data
	if upload.store.ExifRemove {
		if fileType, ok := upload.info.MetaData["filetype"]; ok && (fileType == "image/png" || fileType == "image/jpeg") {
			...
			upload.info.Size = int64(len(noExifBytes))
		}
	}
